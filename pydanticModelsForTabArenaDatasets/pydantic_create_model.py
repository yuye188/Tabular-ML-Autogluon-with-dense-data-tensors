...
def map_dtype(col, dtype, df):
    ...
    if dtype_str == "int64":
        ...
        return (
            Optional[Annotated[NonNegativeInt, Field(lt=255)]]
            if df[col].isnull().any()
            else Annotated[NonNegativeInt, Field(lt=255)]
        )
        ...
        return (
            Optional[datetime.datetime] if df[col].isnull().any() else datetime.datetime
        )
        ...
        enum_type = create_enum_from_categories(
            f"{col.capitalize()}Enum", df[col].cat.categories
        )
...
def create_pydantic_model(model_name, df):
    ...
    fields = {col: (map_dtype(col, dtype, df), ...) for col, dtype in df.dtypes.items()}
